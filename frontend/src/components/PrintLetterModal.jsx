...
const renderLetterHtml = (target, lang, origin) => {
  const text = getLetterText(lang, target.ds_name)
  const headerHtml = renderLetterHeader(origin)
  const headerCss = letterHeaderStyles()
  ...
  <body>
    <div class="print-wrapper">
      ...
      <div class="recipient">
        ...
        <div>${target.ds_name} Divisional Secretariat</div>
        <div>${target.district_name} District</div>
        <div>${target.province_name} Province</div>
      </div>
...
export default function PrintLetterModal({ target, language, onClose }) {
  ...
  const text = getLetterText(language, target.ds_name)
  const dateStr = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
        ...
        <div ref={printRef} style={{padding:'35px 40px',fontFamily:'"Times New Roman", Times, serif',color:'#222',fontSize:12,lineHeight:1.5}}>
          <div style={{marginBottom:16}}>
            <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:10}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:10,lineHeight:1.4}}>රාජ්‍ය පරිපාලන, පළාත් සභා සහ පළාත් පාලන අමාත්‍යාංශය</div>
                <div style={{fontSize:10,lineHeight:1.4}}>பொது நிர்வாகம், மாகாண சபைகள் மற்றும் உள்ளாட்சி அமைச்சகம்</div>
                <div style={{fontWeight:700,fontSize:11,lineHeight:1.4,marginTop:2}}>Ministry of Public Administration, Provincial Councils and Local Government</div>
              </div>
            </div>

            <div style={{borderTop:'2px solid #222',borderBottom:'1px solid #222',padding:'5px 0',marginBottom:10}} />

            <div style={{display:'flex',justifyContent:'space-between',gap:12,fontSize:9,lineHeight:1.5}}>
              <div style={{flex:1}}>
          ...
          <div style={{marginBottom:14,fontSize:12}}>{dateStr}</div>
          ...
          <div style={{marginBottom:16}}>
            <p style={{marginBottom:8}}>{text.greeting}</p>
            {text.paragraphs.map((p, i) => (
              <p key={i} style={{marginBottom:8,textIndent:24}}>{p}</p>
            ))}
          </div>
          ...
          <div style={{marginTop:18}}>
            ...
            <div style={{marginTop:28}}>
              ...
              <div style={{fontWeight:700}}>{text.senderName}</div>
              <div style={{fontSize:11}}>{text.senderMinistry}</div>
            </div>
          </div>
        </div>
      </div>
